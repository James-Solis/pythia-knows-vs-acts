...
def run_fact(fact):
    ...
    # ---------- A) ATTRIBUTION ----------
    # Need per-head output and per-MLP output at the final position.
    # cache 'result' = per-head output post-W_O (hook_result), 'mlp_out' = MLP output.
    print("\n[A] Attribution: logit-lens of each component's OUTPUT contribution")
    _, cache = model.run_with_cache(
        toks,
        names_filter=lambda n: ("hook_result" in n or "mlp_out" in n or "resid_post" in n),
    )

    attrib_rows = []
    for L in ANALYZE_LAYERS:
        # Per-head: hook_result shape [batch, seq, n_heads, d_model]
        head_out = cache["result", L][0, -1]  # [n_heads, d_model]
